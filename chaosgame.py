...
def main(mode, width, height, n, r):
    chaosgame.init()
    screen = chaosgame.display.set_mode((width, height))
    screen.fill(background_color)
    chaosgame.display.set_caption('Chaos Game')
    ...
    while run:
        ...
        for i in range(0, width * height):
            ...
            step += 1
            font = chaosgame.font.Font(None, 24)
            text = font.render("%d " % (step), True, text_color, background_color)
            screen.blit(text, (10, 10))
            chaosgame.display.update()

            for event in chaosgame.event.get():
                if event.type == chaosgame.QUIT:
                    chaosgame.quit()
                    return
